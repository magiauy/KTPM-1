...
public class TenantBUS {
    ...
    public boolean update(Tenant tenant){
        ...
        if (check){
            this.listTenant.set(getIndexByTenantID(tenant.getTenantID()), tenant);
        }
        return check;
    }

    public int getIndexByTenantID(String tenantID){
        int i=0;
        int vitri=-1;
        while (i < this.listTenant.size() && vitri == -1){
            if (listTenant.get(i).getTenantID().equals(tenantID)){
                vitri = i;
            } else {
                i++;
            }
